...
const checkConfigAndReinitialize = () => {
  const config = store.state.environment.config;
  if (config && Object.keys(config).length > 0 && !isConfigLoaded) {
    console.log(
      "Environment config loaded, reinitializing request instance..."
    );
    reinitializeRequest();
    isConfigLoaded = true;
  }
...
store.subscribe((mutation) => {
  if (mutation.type === "environment/SET_CONFIG") {
    checkConfigAndReinitialize();
  }
});
